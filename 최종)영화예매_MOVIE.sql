...
DESC reservation_information;
CREATE TABLE reservation_information (
    reservation_id INT PRIMARY KEY,        -- 예매 번호
    movie_id INT,                          -- 영화 ID (영화 테이블의 외래 키)
    user_id VARCHAR2(50),                  -- 사용자 ID (사용자 테이블의 외래 키)
    reservation_date TIMESTAMP,            -- 예매 일시
    seat_number VARCHAR2(10),              -- 좌석 번호
    reservation_status VARCHAR2(20)       -- 예매 상태 (예: 완료, 취소, 대기)
);
